...
之前我们说 `?` 就是要么 `unwrap` 要么 `return Err(error)`，这大部分是对的，但
事实上 `?` 是要么 `unwrap` 要么 `return Err(From::from(err))`。`From::from` 是
不同类型间的转换工具，也就是说如果你在错误能够转换成返回类型的地方使用 `?`，它
会自动转换。
...
这段代码现在已经很清晰了。相比原始的 `panic`，它非常像是把所有的 `unwrap` 调用
都换成 `?`。与 `panic` 相比，这样做的区别在于返回类型是 `Result`，因而必须在顶层
解构它们。
